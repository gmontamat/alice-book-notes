...
of classes. We could regress on a real
value $\tilde{y}_i \in \lbrack 1, m \rbrack$ and map back to the original domain
by rounding to the nearest integer. This is not a good modeling choice because
we're introducing a spurious ordering of classes that can be exploited by the
model itself (e.g. class 2 is closer to 3 than to class 4). Instead, we use
the **one-hot encoded** version of $y$,
